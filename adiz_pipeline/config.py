...
MIN_POLYGON_AREA = 180  # 最小多邊形面積（像素²）
MAX_POLYGON_AREA = 15000  # 最大多邊形面積（排除過大雜訊）
MAX_POLYGON_AREA_RATIO = 0.5  # 最大面積佔圖比例
MAX_POLYGON_CIRCULARITY = 0.88  # 最大圓形度（排除圓形標點，放寬以納入圓角矩形）
POLYGON_APPROX_EPS = 0.025  # approxPolyDP 誤差係數（略放寬以保留更多頂點）

# 線段偵測參數（僅 86248 等少數圖有路徑線，需嚴格避免誤檢多邊形邊框）
MIN_LINE_LENGTH = 130  # 最小線段長度（像素）
MAX_LINE_GAP = 4  # 虛線合併時最大間距（像素）
DASHED_ANGLE_TOLERANCE = 10  # 虛線段方向容忍（度）
MIN_PATH_LENGTH = 100  # 合併後路徑最小總長（像素），僅保留長路徑
LINE_EXCLUDE_DILATE = 10  # 多邊形/標點排除時的膨脹核大小

# 標點偵測參數（實圖標點較小且可能含數字）
MIN_MARKER_AREA = 25  # 最小標點面積（像素²）
MAX_MARKER_AREA = 7000  # 最大標點面積（像素²）
MIN_MARKER_CIRCULARITY = 0.28  # 最小圓形度
...
MAP_ROI_BOTTOM = 1.0
# 線段偵測額外排除左側（表格可能延伸）
LINE_ROI_LEFT = 0.32  # 線段僅在 x >= 32% 搜尋，避免表格邊框
